  ...
  for (const idp of ['singPass', 'corpPass']) {
    ...
    app.get(`/${idp.toLowerCase()}/metadata`, (req, res) => {
      ...
      if (idp === 'corpPass') {
        ...
        ] = `${baseUrl}/${idp.toLowerCase()}/authorization-info`
      }
    ...
    app.post(
      ...
      async (req, res) => {
        let { client_id: aud, grant_type: grant } = req.body
        let profile, nonce
        ...
        // Retrieve aud from client_assertion when client_id doesn't exist in the request body
        const clientAuthToken = req.body.client_assertion
